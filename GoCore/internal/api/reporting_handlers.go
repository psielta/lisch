...
func (api *Api) handleGetPedido(w http.ResponseWriter, r *http.Request) {
	// 1. Lê a base URL da .env
	baseURL := os.Getenv("GOBID_FAST_API")
	if baseURL == "" {
		http.Error(w, "GOBID_FAST_API não configurado", http.StatusInternalServerError)
	...
	// 2. Monta a URL completa (assegurando-se de não duplicar '/')
	url := fmt.Sprintf("%s/api/Pedido/GetReportById/%s", strings.TrimRight(baseURL, "/"), id)
...
func (api *Api) handleGetPedidoDif(w http.ResponseWriter, r *http.Request) {
	// 1. Lê a base URL da .env
	baseURL := os.Getenv("GOBID_FAST_API")
	if baseURL == "" {
		http.Error(w, "GOBID_FAST_API não configurado", http.StatusInternalServerError)
	...
	// 2. Monta a URL completa (assegurando-se de não duplicar '/')
	url := fmt.Sprintf("%s/api/Pedido/GetReportDifById/%s", strings.TrimRight(baseURL, "/"), id)
